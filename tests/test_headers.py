...
def test_jwt_custom_headers(Authorize):
    ...
    pair_token = Authorize.create_access_token(subject=1, headers={"access": "bar", "refresh": "foo"})
    ...
    # Pair
    assert Authorize.get_unverified_jwt_headers(pair_token)["access"] == "bar"
    assert Authorize.get_unverified_jwt_headers(pair_token)["refresh"] == "foo"
